...
public class SpeechBubble 
{
    ...
    static public ArrayList<SpeechBubble> calculateWordWrappedPages(PointI resolution, String[] pages, IMeasureTextWidthAndHeight context, RectI headRect, PointI downwardTailOffset, PointI upwardTailOffset)
    {
        ...
        // 5a. initial calculation.
        RectI minToHoldAllPages = new RectI (headMP.getX() - (maxTextWidth/2)+downwardTailOffset.getX(), headRect.getTop()-theMostLines*heightPerLine-BUFFER_BOTTOM-avoidSharpPinToHead+downwardTailOffset.getY(), maxTextWidth, theMostLines*heightPerLine);
        
        // 5b. if offscreen vertical, then completely recalculate, and set isPointingDown = true;
        boolean isPointingDown = true;
        if(minToHoldAllPages.getTop()<0)
        {
            isPointingDown = false;
            minToHoldAllPages = new RectI (headMP.getX() - (maxTextWidth/2)+upwardTailOffset.getX(), headRect.getBottom()+BUFFER_BOTTOM+avoidSharpPinToHead+upwardTailOffset.getY(), maxTextWidth, theMostLines*heightPerLine);
        }
        
        // if we are offscreen left or right - easier: make them flush to either left or right side. 
        // we ignore the xoofset here.
        ...
        for(int i=0;i<toReturn.size();i++)
        {
            ...
            RectI max = page.rectBubble;
            {
                ...
                page.isPointingDown = isPointingDown;//headCentre.getY() > centre.getY();
